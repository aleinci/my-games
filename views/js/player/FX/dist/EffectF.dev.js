...
function dibujarFuegoP() {
  for (i in pFuego) {
    ...
    ctx.rotate(pV.angle); //ctx.fillRect(pV.width/-2, pV.height/-2, pV.width, pV.height);
...
function MovimientoFuegoP() {
  for (i in pFuego) {
    var pV = pFuego[i]; //pV.y-=1;

    pV.contX++;
    pV.y += Math.sin(play.y);
    pV.x += Math.cos(play.x);
    pV.x -= pV.speed + 5 * Math.sin(pV.angle) +
    /*obsTimeSp*/
    velocityO * 0.7;
    pV.y += pV.speed * Math.cos(pV.angle); //if (play.width/2 + play.x > pV.x) { pV.x -= pV.speed * Math.sin(pV.angle); }
