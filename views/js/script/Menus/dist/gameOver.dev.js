...
function reiniciar() {
  ...
  ctx.globalAlpha = me;
  ctx.fillStyle = "green";
  ctx.drawImage(menuScore, x, y + 70, width, height - 150); // imagen GAME OVER

  ctx.drawImage(gameO, canvas.width / 2 - gameO.width / 2, y, gameO.width, gameO.height);
  ctx.fillStyle = "white"; //BOTON reiniciar

  ctx.drawImage(btnGO, btnGO.width / 2 * botons.step, 0, btnGO.width / 2, btnGO.height / 2, botons.x, botons.y, botons.width, botons.height); //boton salir al menu

  ctx.drawImage(btnGO, btnGO.width / 2 * botons2.step, 0 + btnGO.height / 2, btnGO.width / 2, btnGO.height / 2, botons2.x, botons2.y, botons2.width, botons2.height);
  ...
  ctx.restore(); //boton de reiniciar juego

  if (cX > botons.x && cX < botons.x + botons.width && cY > botons.y && cY < botons.y + botons.height && mouse == true && me >= 1 && sabiasQ == false) {
    botons.press = true;
  }

  if (botons.press && mouse == false) {
    //restablecer valores y reiniciar juego
    botons.press = false;
    retry = true;
    reinicio();
  } else {
  ...
  if (cX > botons2.x && cX < botons2.x + botons2.width && cY > botons2.y && cY < botons2.y + botons2.height && mouse == true && me >= 1 && sabiasQ == false) {
    botons2.press = true;
  }

  if (botons2.press && mouse == false) {
    //restablecer valor e ir al menu
    botons2.press = false;
    mMenu = !mMenu;
    reinicio();
  }
  ...
  if (sabiasQ) {
    ...
    if (cX > quitBtn.x && cX < quitBtn.x + quitBtn.width && cY > quitBtn.y && cY < quitBtn.y + quitBtn.height && mouse && me >= 1) {
      contadorRSQ = 0;
      sabiasQ = !sabiasQ;
    }
  }
...
function scrStg() {
  switch (mapN) {
    ...
    default:
      ctx.fillText(record0, canvas.width / 2 + 50, canvas.height / 3 + 144);
    //RECOR	D
  }
}