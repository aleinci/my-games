...
function pausaBoton() {
	...
	if (cX > startBtn.x && // boton de pausa
		cX < startBtn.x + startBtn.width &&
		cY > startBtn.y &&
		cY < startBtn.y + startBtn.height && play.estado == "vivo") {
...
function pausa() {
	...
	ctx.drawImage(btnPausa, 0, 0, btnPausa.width/2, btnPausa.height/2,
		continued.x, continued.y, continued.width-10, continued.height-10);// reanudar

	ctx.drawImage(btnPausa, 0, btnPausa.height/2, btnPausa.width/2, btnPausa.height/2,
		salir.x, salir.y, salir.width, salir.height);// salir
	ctx.restore();
	
	if (cX > salir.x && //boton de salir
		cX < salir.x + salir.width &&
		cY > salir.y &&
		cY < salir.y + salir.height) {
	...
	if (cX > continued.x && // boton de reanudar
		cX < continued.x + continued.width &&
		cY > continued.y &&
		cY < continued.y + continued.height) {
